...
import { 
  ...
  CreditCard, Package, ArrowRight, LayoutGrid, List, Receipt, Tag
} from 'lucide-react';
...
const S = {
  ...
  partCard: { background: 'var(--bg-card)', border: '1.5px solid var(--border-color)', borderRadius: '20px', padding: '1.5rem', transition: 'all 0.3s ease' },
...
const SalesAndInvoice = () => {
  const partsInventory = [
    { id: 1, name: 'Engine Oil (5L)', price: 4500, stock: 25, category: 'Maintenance' },
    { id: 2, name: 'Brake Pads - Rear', price: 1200, stock: 15, category: 'Braking' },
    { id: 3, name: 'Air Filter', price: 850, stock: 9, category: 'Maintenance' },
    { id: 4, name: 'LED Headlamp', price: 2100, stock: 40, category: 'Electrical' },
    { id: 5, name: 'Fuel Filter', price: 1450, stock: 12, category: 'Maintenance' },
    { id: 6, name: 'Spark Plug (Set)', price: 3200, stock: 4, category: 'Engine' },
  ];
    ...
    <div style={S.page} className="page-transition">
      <div style={S.header}>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          ...
          <button className="btn btn-primary"><CheckCircle size={18} /> Finalize Session</button>
        </div>
      </div>
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2.5rem' }}>
             ...
             <div style={{ position: 'relative' }}>
                ...
                <input className="input" placeholder="Scan or type component name..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{ padding: '10px 10px 10px 38px', width: '320px', fontSize: '13px' }} />
             </div>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1.25rem', marginBottom: '3.5rem' }}>
             {filteredParts.map(part => (
               <div key={part.id} style={S.partCard} className="hover:border-primary group">
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem' }}>
                     <span style={{ fontSize: '10px', fontWeight: 800, color: 'var(--text-muted)', textTransform: 'uppercase' }}>{part.category}</span>
                     <button onClick={() => addToCart(part)} style={{ background: 'var(--primary)10', border: 'none', width: '32px', height: '32px', borderRadius: '80px', color: 'var(--primary)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <Plus size={18} />
                     </button>
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                     ...
                     <span style={{ fontSize: '10px', fontWeight: 800, color: part.stock < 10 ? '#ef4444' : 'var(--text-muted)' }}>STOCK: {part.stock} UNITS</span>
                  </div>
               </div>
             ))}
          </div>
                   ...
                   {cart.length === 0 ? (
                     ...
                     <tr key={item.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                        <td style={S.td}>
                           ...
                           <div style={{ fontSize: '10px', color: 'var(--text-muted)', textTransform: 'uppercase' }}>{item.category}</div>
                        </td>
        ...
        <aside>
          <div style={{ background: 'var(--bg-card)', border: '1.5px solid var(--border-color)', borderRadius: '24px', padding: '1.5rem', marginBottom: '1.5rem' }}>
             ...
             <div style={{ marginTop: '1rem', padding: '12px', background: 'var(--primary)05', borderRadius: '10px', border: '1px solid var(--primary)20', display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <span style={{ fontSize: '11px', fontWeight: 800, color: 'var(--primary)' }}>LOYALTY ACTIVE: PLATINUM</span>
             </div>
          </div>
          ...
          <div style={S.summaryCard}>
             ...
             <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                ...
                {discount > 0 && (
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '14px', color: 'var(--primary)' }}>
                     <span style={{ display: 'flex', alignItems: 'center', gap: '8px' }}><Zap size={14} fill="var(--primary)" /> Smart Reward (-10%)</span>
             ...
             <div style={{ margin: '2rem 0', paddingTop: '2rem', borderTop: '1px dashed rgba(255,255,255,0.1)' }}>
                ...
                <h2 style={{ fontSize: '2.8rem', fontWeight: 800, color: 'var(--primary)' }}>Rs. {total}</h2>
             </div>
             ...
             <button className="btn btn-primary" style={{ width: '100%', background: 'var(--primary)', color: '#fff', padding: '16px', fontSize: '14px' }}>
                <Mail size={18} /> Finalize & Dispatch <ArrowRight size={18} />
             </button>
